from dnc_site_full.services.emails.email_sender_gmail import EmailSenderGmail
from dnc_site_full.services.emails.email_sender_sendgrid import EmailSenderSandgrid
...
class EmailProvider:
    ...
    def __init__(self):
        ...
        self.sent_count_sendgrid = 0

    def send_email(self, to, subject, text, provider=SENDGRID_PROVIDER):
        ...
        elif provider == EmailProvider.SENDGRID_PROVIDER:
            EmailSenderSandgrid.send_email_via_sendgrid(to, subject, text)
            self.sent_count_sendgrid = self.sent_count_sendgrid + 1
        else:
    ...
    def send_email_list(self, to_list, subject, text, provider=GOOGLE_PROVIDER):
        ...
        if provider == EmailProvider.GOOGLE_PROVIDER:
            ...
            EmailSenderSandgrid.send_email_via_sendgrid_list(to_list, subject, text)
            self.sent_count_sendgrid = self.sent_count_sendgrid + len(to_list)
        else:
            raise AttributeError("please pick on of the providers we have")
    ...
    def get_sent_count_sendgrid(self):
        return self.sent_count_sendgrid
